 ...
 * **************************************************************************/
int execute_Inquiry(int fd,int address, struct sg_io_hdr * p_hdr) {
	unsigned char cdb[10]={0x2a,0,0,0,0,0,0,0,1,0};

	/* set the cdb format 
	cdb[0] = 0x; This is for Inquery
	cdb[1] = evpd & 1;
	cdb[2] = page_code & 0xff;
	cdb[3] = 0;
	cdb[4] = 0xff;
	cdb[5] = 0; For control filed, just use 0*/
	cdb[5]=(unsigned char) (address);
	cdb[4]=(unsigned char) (address >> 8);
	cdb[3]=(unsigned char) (address >> 16);
	cdb[2]=(unsigned char) (address >> 24);
